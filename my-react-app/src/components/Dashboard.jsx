...
import { getStatsSummary, getAssessments } from '../api/assessments';
...
const StudentDashboard = ({ onLogout, onNavigate }) => {
  ...
  // New: real recent assessments, replacing the old 3 hardcoded sample rows.
  ...
  useEffect(() => {
    ...
    const loadRecent = async () => {
      try {
        // Ask the backend for just the 3 most recently created assessments.
        const result = await getAssessments({ sortBy: 'date', order: 'desc', page: 1, limit: 3 });
        setRecent(result.data);
      } catch (err) {
        console.error('Failed to load recent assessments:', err);
        setRecentError('Could not load recent assessments.');
      } finally {
  ...
  const statCards = [
    { label: 'Total Cases',         value: stats.total,    sub: 'All Assessments', accent: '#2563eb' },
    { label: 'Low Risk Cases',      value: stats.low,      sub: percentLabel(stats.low, stats.total),      accent: '#16a34a' },
    { label: 'Moderate Risk Cases', value: stats.moderate, sub: percentLabel(stats.moderate, stats.total), accent: '#d97706' },
    { label: 'High Risk Cases',     value: stats.high,     sub: percentLabel(stats.high, stats.total),     accent: '#dc2626' },
  ];
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '24px 28px' }}>
          ...
          {statsError && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '12px 16px', marginBottom: '16px', backgroundColor: '#fef2f2', border: '1.5px solid #fecaca', borderRadius: '10px' }}>
              <span style={{ fontSize: '13px', color: '#dc2626', fontWeight: '600' }}>{statsError}</span>
            </div>
          )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '16px', marginBottom: '24px' }}>
            {statCards.map(card => (
              <div key={card.label} style={{ backgroundColor: '#fff', borderRadius: '12px', padding: '18px 20px', boxShadow: '0 1px 4px rgba(0,0,0,0.07)', borderTop: `3px solid ${card.accent}` }}>
                ...
                <p style={{ margin: '0 0 4px', fontSize: '36px', fontWeight: '800', color: '#1e293b', lineHeight: 1 }}>
          ...
          {/* Recent Assessments — now real data */}
          <div style={{ backgroundColor: '#fff', borderRadius: '12px', boxShadow: '0 1px 4px rgba(0,0,0,0.07)', overflow: 'hidden' }}>
            ...
            {loadingRecent ? (
              <div style={{ padding: '30px', textAlign: 'center', color: '#94a3b8', fontSize: '13px' }}>Loading recent assessments...</div>
            ) : recentError ? (
              <div style={{ padding: '30px', textAlign: 'center', color: '#dc2626', fontSize: '13px' }}>{recentError}</div>
            ) : recent.length === 0 ? (
              ...
              recent.map((row, i) => (
                <div key={row._id}
                  style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1.5fr 2fr 1fr', padding: '14px 20px', alignItems: 'center', borderBottom: i < recent.length - 1 ? '1px solid #f8fafc' : 'none', transition: 'background 0.1s' }}
                  onMouseOver={e => e.currentTarget.style.background = '#f8fafc'}
                  onMouseOut={e => e.currentTarget.style.background = 'transparent'}
                >
                  <span style={{ fontSize: '14px', fontWeight: '600', color: '#1e293b' }}>{row.patientName}</span>
                  <span style={{ fontSize: '14px', color: '#374151' }}>{row.age}</span>
                  <span><span style={{ display: 'inline-block', padding: '3px 10px', borderRadius: '20px', backgroundColor: riskBg[row.riskLevel], color: riskColor[row.riskLevel], fontSize: '12px', fontWeight: '700' }}>{row.riskLevel}</span></span>
                  <span style={{ fontSize: '13px', color: '#475569' }}>{formatDate(row.createdAt)}</span>
                  <span>
