  ...
  return (
    <div style={{
      ...
      {showResetModal && (
        <div style={{
          ...
          <div style={{
            ...
            {resetError && (
              <p style={{ margin: '0 0 16px', fontSize: '12px', color: '#dc2626' }}>{resetError}</p>
            )}
            
            {resetMessage && (
              <p style={{ margin: '0 0 16px', fontSize: '12px', color: '#16a34a' }}>{resetMessage}</p>
            )}
            
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
              <button onClick={() => { setShowResetModal(false); setResetEmail(''); setResetError(''); setResetMessage(''); }}
                style={{ padding: '10px 20px', borderRadius: '8px', border: '1.5px solid #d1d5db', background: '#fff', fontSize: '14px', fontWeight: '600', color: '#374151', cursor: 'pointer' }}>
                Cancel
              </button>
              <button onClick={handlePasswordReset} disabled={loading}
                style={{ padding: '10px 20px', borderRadius: '8px', border: 'none', background: 'linear-gradient(135deg,#2563eb,#1d4ed8)', fontSize: '14px', fontWeight: '600', color: '#fff', cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1 }}>
      ...
      <div style={{
        ...
        overflow: 'hidden', display: 'flex', minHeight: '500px'
      }}>
        ...
        <div style={{ flex: 1, padding: '48px 44px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          {/* Changed from "Welcome Back!" to "Welcome" */}
          ...
          <form onSubmit={handleSubmit} noValidate style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <button type="button" onClick={() => setShowResetModal(true)}
                style={{ fontSize: '13px', color: '#2563eb', textDecoration: 'none', fontWeight: '500', background: 'none', border: 'none', cursor: 'pointer' }}
            ...
            {/* Register link - Changed from "Register" to "Sign up" */}
